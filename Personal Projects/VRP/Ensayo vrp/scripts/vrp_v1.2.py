...
def create_map(df, routes, depots, vehicle_depot_assignments):
    """
    ...
    for i, (route, _) in enumerate(routes):
        ...
        start_depot = vehicle_depot_assignments[i]
        route_points = [depots[start_depot]]
        ...
        route_points.append(depots[start_depot])
...
def optimize_routes(file_path, depots, vehicles_per_depot, capacity_per_vehicle, is_cvrp=True, consider_depot_distance=True):
    """
    ...
    try:
        ...
        # Create vehicle to depot assignments
        vehicle_depot_assignments = [depot for depot, count in enumerate(vehicles_per_depot) for _ in range(count)]
        ...
        data = {
            ...
            'depot': 0,
            'starts': vehicle_depot_assignments,
            'ends': vehicle_depot_assignments
        }
        ...
        if solution:
            ...
            # Create Folium map
            m = create_map(df, routes, depots, vehicle_depot_assignments)
            ...
            for i, (route, distance) in enumerate(routes):
                ...
                output_data.append({
                    ...
                    'Start Depot': start_depot + 1,  # Adding 1 for human-readable depot numbering
...
for i in range(num_depots):
    ...
    vehicles = int(input(f"Enter number of vehicles for depot {i+1}: "))
    vehicles_per_depot.append(vehicles)

capacity_per_vehicle = int(input("Enter the capacity per vehicle: "))
consider_depot_distance = input("Do you want to consider depot distance (y/n)? ").lower() == 'y'

# Run optimization
optimize_routes(file_path, depots, vehicles_per_depot, capacity_per_vehicle, is_cvrp=True, consider_depot_distance=consider_depot_distance)