...
def create_map(df, routes, depot_lat, depot_lon):
    ...
    for i, route in enumerate(routes):
        ...
        # Add route line from depot to first point
        folium.PolyLine(
        ...
        # Add route line from last point to depot
        folium.PolyLine(
...
def run_optimization():
    ...
    try:
        ...
        if solution:
            ...
            save_results(df, routes, depot_lat, depot_lon, 'output1.xlsx', 'output2.xlsx', include_start_distance, include_end_distance)
...
tk.Checkbutton(root, text="Include distance from depot to first point", variable=include_start_distance_var).grid(row=7, column=0, columnspan=2)

include_end_distance_var = tk.BooleanVar(value=True)
tk.Checkbutton(root, text="Include distance from last point to depot", variable=include_end_distance_var).grid(row=8, column=0, columnspan=2)

tk.Button(root, text="Run Optimization", command=run_optimization).grid(row=9, column=0, columnspan=3)
