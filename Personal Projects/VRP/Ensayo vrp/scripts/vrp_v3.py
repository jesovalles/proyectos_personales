...
class RoutingOptimizer:
    ...
    def optimize_routes(self, df, depot_coords, is_cvrp, use_all_vehicles, consider_depot_distance, random_seed):
        ...
        unique_vehicles = df['NombreVahiculo'].unique()
        num_vehicles = len(unique_vehicles) if use_all_vehicles else min(len(unique_vehicles), len(df) // 6)
        ...
        data = {
            ...
            'depot': 0,
            'vehicle_names': unique_vehicles[:num_vehicles]  # Limit to the number of vehicles we're using
        }
    ...
    def get_solution_details(self, data, manager, routing, solution, consider_depot_distance):
        ...
        for vehicle_id in range(data['num_vehicles']):
            ...
            routes.append((route, route_distance, data['vehicle_names'][vehicle_id]))
    ...
    def create_output(self, result, df, depot_coords):
        ...
        # Create Excel output
        output_data = []
        for i, (route, distance, vehicle_name) in enumerate(routes):
        ...
        output_df.to_excel('route_summary33333.xlsx', index=False)
        ...
        # Add routes
        for i, (route, _, vehicle_name) in enumerate(routes):
            ...
            for j, point in enumerate(route[1:-1]):
                if point >= len(depot_coords):
                    folium.Marker(
                        ...
                        popup=f'Delivery Point {j+1} (Vehicle {vehicle_name})',
        ...
        m.save('route_map_333333.html')
        webbrowser.open('route_map_333333.html', new=2)
